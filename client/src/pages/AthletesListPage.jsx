...
import { Search, Plus, SlidersHorizontal, Copy, AlertTriangle } from 'lucide-react';
...
export default function AthletesListPage() {
  ...
  const params = useMemo(() => ({
    ...
    passportExpiring: passportExpiring || undefined,
  }), [page, search, status, missing, travelWindow, passportExpiring]);
  ...
  async function handleDuplicate(e, athleteRow) {
    e.stopPropagation(); // don't trigger the row's navigate-to-detail click
    // The list row is a summary - fetch the full record so the form has
    // everything to pre-fill (notes, embassy, visa type, etc).
    const detail = await athleteApi.detail(athleteRow.id);
    setDuplicateFrom(detail.athlete);
    setFormOpen(true);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {showFilters && (
        <div className="grid grid-cols-1 gap-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-surface dark:bg-surface-dark p-4 sm:grid-cols-4">
          <div>
          ...
          <div className="flex items-end pb-2">
            <label className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
              <input type="checkbox" checked={passportExpiring} onChange={(e) => { setPassportExpiring(e.target.checked); setPage(1); }} className="rounded border-slate-300 dark:border-slate-600" />
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-surface dark:bg-surface-dark">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {loading && (
              <tr><td colSpan={8} className="px-4 py-8 text-center text-slate-400">Loading athletes\u2026</td></tr>
            )}
            {error && (
              <tr><td colSpan={8} className="px-4 py-8 text-center text-status-action">{error}</td></tr>
            )}
            {!loading && !error && data.length === 0 && (
              <tr><td colSpan={8} className="px-4 py-8 text-center text-slate-400">No athletes match these filters.</td></tr>
            )}
            {!loading && data.map((a) => (
              <tr
                key={a.id}
      ...
      <AthleteFormModal
        open={formOpen}
        onClose={() => { setFormOpen(false); setDuplicateFrom(null); }}
        onSaved={(result) => {
          if (duplicateFrom && result?.id) navigate(`/athletes/${result.id}`);
          else refetch();
        }}
        athlete={null}
